 ...
 * Screens call this. They never touch a repository directly.
 */
export function usePrescriptions(filters: PrescriptionFilters = {}) {
  ...
  return useInfiniteQuery({
    ...
    // Changing a filter changes the query key, which would otherwise drop the
    // list back to its loading state on every keystroke. This keeps the last
